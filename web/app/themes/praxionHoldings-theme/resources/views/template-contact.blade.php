...
@php
    ...
    $contactErrors =
        isset($contactResult['errors']) && is_array($contactResult['errors']) ? $contactResult['errors'] : [];
    $contactValues =
        isset($contactResult['values']) && is_array($contactResult['values']) ? $contactResult['values'] : [];
                ...
                <div
                    ...
                    <div aria-live="polite" aria-atomic="true">
                        @if ($contactStatus === 'success')
                            <div class="mb-7 flex items-start gap-3 rounded-2xl border border-green-300 bg-green-50 p-4 text-green-950"
                                role="status">
                        ...
                        @elseif ($contactStatus === 'error')
                            <div class="mb-7 rounded-2xl border border-red-300 bg-red-50 p-4 text-red-950" role="alert"
                                tabindex="-1">
                                        ...
                                        @if ($contactErrors !== [])
                                            ...
                                            <ul class="mt-2 list-disc space-y-1 pl-5 text-sm">
                                                @foreach ($contactErrors as $field => $error)
                                                    <li><a href="#contact-{{ str_replace('_', '-', $field) }}"
                                                            class="font-semibold underline underline-offset-2">{{ $error }}</a>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        @endif
                    ...
                    <form method="post" action="{{ admin_url('admin-post.php') }}" x-data="{ submitting: false }"
                        ...
                        <div class="sr-only" aria-hidden="true">
                            ...
                            <input id="company-website" name="company_website" type="text" tabindex="-1"
                                autocomplete="off">
                        </div>
                        ...
                        <div>
                            ...
                            <input id="contact-full-name" name="full_name" type="text" autocomplete="name" required
                                maxlength="100" value="{{ $fieldValue('full_name') }}"
                                ...
                                @class([
                                    ...
                                    'border-gray-300 focus:border-primary' => !isset(
                                        $contactErrors['full_name']),
                                ])>
                            <p id="full-name-help" class="mt-2 text-xs leading-5 text-muted">Use 100 characters or fewer.
                            </p>
                            @if (isset($contactErrors['full_name']))
                                <p id="full-name-error" class="mt-2 text-sm font-semibold text-red-700">
                                    {{ $contactErrors['full_name'] }}</p>
                            @endif
                        </div>
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <input id="contact-email" name="email" type="email" autocomplete="email"
                                    inputmode="email" required maxlength="254" value="{{ $fieldValue('email') }}"
                                ...
                                <p id="email-help" class="mt-2 text-xs leading-5 text-muted">Enter an address that can
                                    receive a reply.</p>
                                @if (isset($contactErrors['email']))
                                    <p id="email-error" class="mt-2 text-sm font-semibold text-red-700">
                                        {{ $contactErrors['email'] }}</p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <input id="contact-phone" name="phone" type="tel" autocomplete="tel"
                                    inputmode="tel" maxlength="30" value="{{ $fieldValue('phone') }}"
                                ...
                                <p id="phone-help" class="mt-2 text-xs leading-5 text-muted">Use 30 characters or fewer.
                                </p>
                                @if (isset($contactErrors['phone']))
                                    <p id="phone-error" class="mt-2 text-sm font-semibold text-red-700">
                                        {{ $contactErrors['phone'] }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="contact-message" name="message" rows="7" required maxlength="3000"
                                ...
                                @if (isset($contactErrors['message'])) aria-invalid="true" @endif @class([
                            ...
                            <p id="message-help" class="mt-2 text-xs leading-5 text-muted">Use 3,000 characters or fewer.
                                Do not include sensitive information.</p>
                            @if (isset($contactErrors['message']))
                                <p id="message-error" class="mt-2 text-sm font-semibold text-red-700">
                                    {{ $contactErrors['message'] }}</p>
                            @endif
                        </div>
                        ...
                        <p class="sr-only" aria-live="polite" x-text="submitting ? 'Your message is being sent.' : ''">
                        </p>
                    </form>
                </div>
            </div>
