...
<section class="container-page mt-20 sm:mt-24 lg:mt-32 mb-20 sm:mb-24 lg:mb-32" aria-label="Next step">
    <div
        class="relative isolate overflow-hidden rounded-3xl bg-linear-to-br from-primary to-primary-dark px-6 py-14 text-center text-white sm:rounded-4xl sm:px-10 sm:py-18 lg:px-20 lg:py-20">
        <div class="absolute -top-32 -right-24 -z-10 h-80 w-80 rounded-full bg-white/10 blur-3xl" aria-hidden="true">
        </div>

        <p
            class="inline-flex rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold backdrop-blur">
            {{ $eyebrow }}</p>
        <h2 class="mx-auto mt-7 max-w-3xl text-3xl leading-tight font-bold tracking-tight sm:text-4xl lg:text-5xl">
            {{ $title }}</h2>
        <p class="mx-auto mt-5 max-w-2xl text-base leading-7 text-white/85 sm:text-lg sm:leading-8">{{ $description }}
        </p>
